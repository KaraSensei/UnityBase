...
- Приватное поле `WeaponData weaponData` с доступом через свойство `WeaponData` (инкапсуляция данных).
- Поле-владелец `Transform owner` (кто держит это оружие — игрок/враг) с доступом через свойство `Owner`:
  - в этом проекте смены владельца (враг → игрок) нет,
  - но если вы будете делать подбираемое оружие, через `Owner` можно передавать один и тот же экземпляр оружия разным персонажам.
...
Обрати внимание: здесь мы сразу используем **инкапсуляцию** — данные храним в приватных полях, а наружу показываем только свойства:
...
public abstract class WeaponBase : MonoBehaviour
{
    ...
    /// <summary>
    /// Публичный доступ к WeaponData с возможностью смены в рантайме.
    /// Данные остаются инкапсулированы в приватном поле weaponData.
    /// </summary>
    ...
    /// В этом проекте смены владельца (враг → игрок) нет,
    /// но если вы будете делать подбираемое оружие, через это свойство можно
    /// передавать один и тот же экземпляр оружия разным персонажам.
    /// </summary>
...
- Поля `weaponData` и `owner` сделаны приватным/защищённым `[SerializeField]` + свойствами:
  - так мы сохраняем **инкапсуляцию** (данные не торчат наружу как public‑поля),
  - но при этом их удобно настраивать в инспекторе.
