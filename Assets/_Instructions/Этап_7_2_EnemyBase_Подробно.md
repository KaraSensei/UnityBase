...
### 0.4. Теория: теги и упрощённый поиск игрока

Для базового потока (12–17 лет, смешанная группа) мы **не вводим слои и сложную физику** сразу, чтобы не перегружать урок.
...
- **Теги (Tags)** — текстовые ярлыки для объектов (например, `Player`, `Enemy`).  
  ...
  - Тег удобен, когда нам нужно понять **«кто это»** логически. В этом уроке мы храним тег игрока в поле `playerTag` и ищем объект игрока по этому тегу.
- **Простой поиск по тегу** — `GameObject.FindWithTag("Player")`:
  - один раз в `Start()` (или по таймеру) ищем объект игрока;
  - сохраняем его `Transform` в поле `target`;
  - дальше используем только расстояние (`Vector3.Distance`) для решения, идти к цели или атаковать.

> В продвинутой дорожке (для старших/сильных учеников) можно добавить отдельный блок по **слоям (Layers)** и `Physics.OverlapSphere`/`Raycast`, показав, как враг может «видеть» игрока с учётом препятствий. Для базовой версии врагов это не обязательно.
